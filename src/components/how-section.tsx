...
const methods = [
  {
    ...
    icon: Smartphone,
    title: '화면 켜서 문 열기',
    desc: '스마트폰 화면을 켜기만 하면 공동현관문이 바로 열립니다.',
  },
  {
    tag: '방법 2',
    icon: MousePointerClick,
    title: '버튼으로 문 열기',
    desc: '버튼만 누르면 공동현관문을 열 수 있습니다.',
  },
]
      ...
      >
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-8">
              {methods.map((m, i) => (
                <div key={m.tag} className="relative flex gap-4 pb-6 last:pb-0">
                  {i < methods.length - 1 && (
                    <span className="absolute left-6 top-12 -z-10 h-[calc(100%-2rem)] w-px bg-border" aria-hidden="true" />
                  )}
                  <span className="flex size-12 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground">
                    <m.icon className="size-6" aria-hidden="true" />
                  </span>
                  <div className="flex-1 rounded-2xl border border-border/60 bg-card p-5 transition-transform duration-300 hover:-translate-y-1">
            ...
            <div className="mt-6 flex items-start gap-2 rounded-xl bg-secondary/60 p-4 text-body-md text-muted-foreground">
          ...
          <div className="relative order-1 lg:order-2">
            <div className="absolute -inset-4 -z-10 rounded-[2rem] bg-primary/5 blur-xl" />
            <img
              src="/phone-widget.png"
              alt="스마트폰 위젯으로 공동현관문을 여는 앱 화면"
              width={520}
              height={640}
              className="w-full rounded-[8px] border border-border/60 object-cover shadow-lg shadow-primary/5"
            />
          </div>
        </div>
      </div>
